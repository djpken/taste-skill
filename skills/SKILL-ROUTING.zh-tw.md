...
0. [設計旋鈕說明：VARIANCE、MOTION、DENSITY](#0-設計旋鈕說明variantemotiondensity)
...
## 0. 設計旋鈕說明：VARIANCE、MOTION、DENSITY

`taste-skill`（`design-taste-frontend`）在 skill 檔案頂端提供三個 **1–10 數字旋鈕**，控制輸出的視覺風格。在套用任何情境前，先設定這三個值。

### VARIANCE — 排版實驗度（DESIGN_VARIANCE）

控制版面的保守程度：低值傾向居中對稱，高值推向不對稱與現代構圖。

| 值域 | 視覺感受 | 適用場合 |
|-----|---------|---------|
| 1–3 | 居中、對稱、傳統格線 | 企業網站、法律、金融 |
| 4–6 | 輕度不對稱、現代但保守 | 一般 SaaS、開發者作品集 |
| 7–8 | 明顯不對稱、大膽構圖 | 創意代理商、設計師作品集 |
| 9–10 | 實驗性、打破版面慣例 | 藝術裝置、時尚、純創意展示 |

> **旁注**：提高 VARIANCE 不等於「更好看」，而是「更有個性」。若品牌訴求是可信度與專業感，8 不一定優於 5。

---

### MOTION — 動畫深度（MOTION_INTENSITY）

控制動態效果的侵略程度：低值只有細微的懸停反饋，高值包含捲動觸發動畫、視差與磁性游標。

| 值域 | 動態效果 | 適用場合 |
|-----|---------|---------|
| 1–3 | 僅懸停淡入、顏色過渡 | 儀表板、資訊密度高的頁面 |
| 4–6 | 入場動畫、平滑過渡 | 一般登陸頁、行銷網站 |
| 7–8 | 捲動觸發、視差、浮動元素 | 作品集、展示型網站 |
| 9–10 | 磁性游標、重動畫、物理感互動 | 創意工作室、概念展示 |

> **效能提醒**：MOTION 7+ 會引入 scroll listener 與 transform 動畫，在低端裝置可能影響 60fps。若目標使用者在行動端，建議 MOTION ≤ 6。

---

### DENSITY — 每視口資訊量（VISUAL_DENSITY）

控制畫面的「呼吸空間」：低值給予大量留白與焦點感，高值在同一視口塞入更多內容。

| 值域 | 視覺感受 | 適用場合 |
|-----|---------|---------|
| 1–3 | 極寬鬆、編輯感、留白大 | 奢侈品、攝影作品集、wellness |
| 4–5 | 標準登陸頁節奏 | 通用起點 |
| 6–7 | 功能羅列、資訊豐富 | SaaS 功能頁、定價頁 |
| 8–10 | 儀表板密度、多欄 Bento | 資料產品、管理後台、新聞閱讀器 |

> **排版互動**：DENSITY 與 VARIANCE 會互相影響——高 VARIANCE + 高 DENSITY 容易產生視覺混亂；若要走實驗性版面，建議 DENSITY 保持在 4 以下。

---

### 三旋鈕速查組合

| 目標感受 | VARIANCE | MOTION | DENSITY |
|---------|---------|--------|---------|
| 高端消費者（wellness / 奢侈品） | 7 | 6 | 3 |
| 一般 SaaS 主流 | 7 | 6 | 4 |
| 創意代理商 | 9 | 8 | 3 |
| 開發者作品集 | 6 | 5 | 4 |
| 設計師 / 工作室作品集 | 9 | 7 | 3 |
